...
import { AlignLeft, UserRound, Moon, Sun } from 'lucide-react'
import logo from '../assets/images/wink.gif'
import { Link } from 'react-router-dom'
6
const Navbar = ({ darkMode, toggleDarkMode, toggleSidebar }) => {
    return (
        <nav className='flex justify-between py-5 bg-[#F0F8FF]  px-2 sm:px-5 md:px-10 fixed top-0 z-50 w-full items-center border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700'>
            <div className='flex items-center md:gap-4 sm:gap-2 gap-1'>
                ...
                <Link to="/">
                    <div className='flex items-center md:gap-4 sm:gap-2 gap-1 cursor-pointer'>
                        <img src={logo} alt="logo-image" className='w-7' />
                        <h2 className='font-semibold text-2xl text-theme-color dark:text-white font-righteousStatic whitespace-nowrap'>LOGO</h2>
                    </div>
                </Link>
            </div>
            <div className='flex items-center gap-1 md:gap-4'>
                <div>
                    <input type="search" name="search" id="search"
                        placeholder='Search'
                        className='w-32 sm:w-full bg-transparent border-[0.1px] hover:border-theme-color rounded-md px-2 py-1 border-gray-300  focus:outline-none focus:ring-2 focus:ring-blue-600 ' />
                </div>
